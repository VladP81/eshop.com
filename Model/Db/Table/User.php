...
class Model_Db_Table_User extends System_Db_Table
{
    ...
    public function getById($id)
    {
        ...
        $sth = $this->getConnection()->prepare($sql); // обработка от sql-инъекции
        $sth->execute([$id]);                         // отправка на выполнение
        
        $result = $sth->fetchAll(PDO::FETCH_OBJ);   // в результате получаем PDО_oбъект
